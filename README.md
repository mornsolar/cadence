...
An agency-returning swipe checkpoint for short-form video on desktop web. A browser extension for Chrome and Firefox that watches TikTok, Instagram Reels, YouTube Shorts and Facebook Reels, and after a set number of completed swipes pauses the feed and asks one question: *Is this what you meant to be doing?*
...
| Term | Rule | Setting |
|---|---|---|
| Completed swipe (a skip) | The active content card changes, that change follows a scroll, swipe or arrow-key gesture within 1.5 seconds, *and* the video being left had not already played through **twice**. Identity is the content id in the URL, with the video that dominates the viewport as a fallback. A change from either source inside a 400 ms window counts once. Going back counts. Like, comment and share taps and cancelled drags do not change the card, so they do not count. Neither does a clip finishing and looping back to itself — none of these platforms auto-advance, and some do the loop by swapping in a fresh video element, which would otherwise look identical to a real skip. Crucially, since these platforms require a swipe to move on even after a video finishes, swiping onward *after* watching one through is not a skip either — only leaving before it's been watched through does count. It takes two full passes, not one, because a single pass is too weak a signal on short clips: a clip that's only a few seconds long can finish on its own before anyone has actually decided to stay or move on, so one loop alone can't be told apart from someone who was about to skip anyway. Seeing it complete twice is real evidence of staying on purpose. Either way, the transition still keeps the session alive; it just does not count toward the threshold unless it was a skip. | fixed |
...
- "Watched through" relies on the video element's `ended` event, with playback position nearing the reported duration as a fallback for players that loop via the native `loop` attribute, and requires it twice before a swipe stops counting as a skip. A player that never fires either signal (for example, one that reports an infinite or missing duration) would misclassify a full watch as a skip.
- Facebook's reel permalink format (`facebook.com/reel/<id>`) is confirmed from public sources, but the path its own Reels tab routes to when browsing without a specific permalink is not independently verified here. If it turns out to differ from what the adapter expects, the extension simply stays inactive on Facebook rather than miscounting — worth checking against the live site before relying on it there.
