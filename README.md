...
| Term | Rule | Setting |
|---|---|---|
| Completed swipe | The active content card changes. Identity is the content id in the URL, with the video that dominates the viewport as a fallback. A change from either source inside a 400 ms window counts once. Going back counts. Like, comment and share taps and cancelled drags do not change the card, so they do not count. | fixed |
